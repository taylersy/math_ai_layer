...
3. 你的分析要专业、落地。你必须进行【班级情况 + 具体板块知识点之间的联动分析】，先分析该章节知识点的特性与重难点，然后将其与四个学情层级深度融合，重点放在不同分层级的最近发展区诊断和后续宏观组织策略上。切忌脱离具体教学内容泛泛而谈。请按照以下结构输出：
   - ### 一、知识点特性与班级整体态势联动分析
...
【设计规范】
1. **知识点特性与班级整体态势联动分析**：首先深入剖析教师提问中涉及的具体章节或知识点的特性与重难点，然后结合当前班级在四个维度上的分布比例进行联动分析，必须使用 Markdown 表格形式直观展现各层人数。
2. **各层级诊断与教研建议**：使用表格（表头为：层级 | 核心学情特征 | 最近发展区目标 | 在本章节/知识点的专属突破策略）详细列出四个层级在应对该具体学习内容时的策略，切忌脱离具体知识点。
